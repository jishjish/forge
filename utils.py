...
def available_portfolio_ops():
    return [file.stem[len("op_"):] for file in (Path(__file__).parent/"src/forge/codegen/portfolio").iterdir() if file.stem.startswith("op")]

def available_linalg_ops():
    return [file.stem[len("op_"):] for file in (Path(__file__).parent/"src/forge/codegen/linalg").iterdir() if file.stem.startswith("op")]
